...
// 	}
	...
	XModelSurfs* model = (XModelSurfs*)surfs;
...
char* LoadCModel(const char* name)
{
	...
	*(void**)(header.main.buffer + 44) = allocationData;
	...
	for (int i = 0; i < numSurfaces; i++)
	{
		...
		dest[6] = '\xFF'; // fake stream handle for the vertex/index buffer get code to use
	}
...
bool Load_XModelAssetHookFunc(char* xmodel)
{
	...
	{
		XModelSurfs* surfs = *(XModelSurfs**)(xmodel + 72 + (44 * i));

		if (!surfs->surfaces)
		{
			if (!strcmp(CURRENT_ZONE_NAME, "ui_viewer_mp"))
			{
				if (_allowedAssetMap.find(surfs->name) != _allowedAssetMap.end())
				{
					continue;
				}
			}
			...
			*(short*)(xmodel + 70 + (44 * i)) = totalv;
			...
			surfs->numSurfaces = ((XModelSurfs*)newSurfs)->numSurfaces;
			surfs->surfaces = ((XModelSurfs*)newSurfs)->surfaces;
...
void DB_RemoveXModelSurfs(XModelSurfs* model)
{
// 	if (GAME_FLAG(GAME_FLAG_DEDICATED))
// 	{
// 		return;

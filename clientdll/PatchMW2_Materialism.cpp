...
Material* Material_Register(const char* filename)
{
	...
	if (!_stricmp(filename, "preview_so_ghillies"))
	{
		return Material_Obtain(filename, filename);
	}
	...
	if (!_stricmp(filename, "preview_mp_nuked"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "preview_mp_cross_fire"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "preview_mp_bloc"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "preview_mp_cargoship"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "preview_mp_bog_sh"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "preview_mp_killhouse"))
	{
		return Material_Obtain(filename, filename);
	}
	...
	if (!_stricmp(filename, "hud_icon_ak74u"))
	{
		return Material_Obtain(filename, "hud_akd74u");
	}

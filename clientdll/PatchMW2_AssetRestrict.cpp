...
DB_GetXAssetNameHandler_t* DB_GetXAssetNameHandlers = (DB_GetXAssetNameHandler_t*)0x7332A8;

char CanWeLoadAsset(assetType_t type, void* entry)
{
	const char* name = DB_GetXAssetNameHandlers[type](entry);
	if (type == ASSET_TYPE_WEAPON && version == 159)
	{
		// somewhat-workaround for issue 'could not load weapon "destructible_car"' and cars not doing any damage
		if (strcmp(CURRENT_ZONE_NAME_159, "common_tc_mp") && (!strcmp(name, "none") || !strcmp(name, "destructible_car"))) // common_tc_mp also has these
		{
			return 1;
		}

		if (!strcmp(CURRENT_ZONE_NAME_159, "patch_mp"))
		{
			return 0;
		}
	}
...
void DoBeforeLoadAsset(assetType_t type, void** entry)
{
	if (entry)
	{
		if(version = 159)
		{
			AssetRestrict_PreLoadFromMaps(type, *entry, CURRENT_ZONE_NAME_159);
			//AssetRestrict_PreLoadFromExperimental(type, *entry, CURRENT_ZONE_NAME_159);
		}
		else if(version == 184)
		{
			AssetRestrict_PreLoadFromMaps(type, *entry, CURRENT_ZONE_NAME_184);
			//AssetRestrict_PreLoadFromExperimental(type, *entry, CURRENT_ZONE_NAME_184);
		}
	}
}
...
DWORD markAssetHookLoc = 0x5822C0;
DWORD markAssetHookRet = 0x5822C8;

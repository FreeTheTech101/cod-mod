...
{
	Cmd_ExecuteSingleCommand(0, 0, "closemenu menu_xboxlive_partyended");
	Cmd_ExecuteSingleCommand(0, 0, "openmenu popup_reconnectingtoparty");

	joinStarted = Com_Milliseconds();
	joinChallenge = joinStarted;

	const char* message = "getinfo _join%i";

	NET_OutOfBandPrint(NS_CLIENT, adr, message, joinChallenge);
}
